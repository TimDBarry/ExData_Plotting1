...
dt_powerraw$Date <- as.Date(dt_powerraw$Date, format = "%d/%m/%Y") 

#Now copy only 2007-02-01 and 2007-02-02 to a new data.table.
dt_powertarg <- dt_powerraw[Date >= "2007-02-01" & Date <= "2007-02-02",]

#Create new date+time column in character format.
dt_powertarg$DateTime <- paste(dt_powertarg$Date, dt_powertarg$Time)  
...
hist(dt_powertarg$Global_active_power
